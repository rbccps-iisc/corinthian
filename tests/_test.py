...
def get_channel(token):
    ...
    if pool.get(username, 'not found') == 'not found' \
        or not pool.get(username).is_open:
...
def gen_admin():
    ...
    cmd = \
        'docker exec postgres psql -U postgres -c "insert into users values(\'' \
        + admin_id + "','" + hash_string + "',NULL,'" + salt \
        + '\',\'f\',\'t\') ON CONFLICT DO NOTHING"'
...
def share_dev(expected, admin_id, admin_key):
    ...
    for follow_req in response:
        count = count + 1
        log('SHARE FROM DEVICE ' + str(follow_req['to']).split('.')[0]
            + ' TO APP ' + str(follow_req['from']), '')
        share_req = share(admin_id, admin_key,
                          str(follow_req['follow-id']))
        check(share_req, 200)

    assert_exit(count == expected, 'Count= ' + str(count)
                + ' Expected = ' + str(expected))
...
def dev_subscribe_amqp(apps, device_keys, expected):
    ...
    for (device, apikey) in device_keys.items():
        ...
        messages = amqp_subscribe(device, apikey, apps,
                                  message_type='command')
        assert_exit(len(messages) == expected, 'Actual= ' + str(actual)
                    + ' Expected = ' + str(expected))
...
def amqp_subscribe(
    ...
    for i in range(num_messages):
        ...
        (method_frame, header_frame, body) = channel.basic_get(ID
                + message_type)
...
def follow_as_admin(
    ...
    share_dev(apps * devices, admin_id=dev_admin_id,
...
def functional_tests(*args):
    ...
    # AMQP tests
    # amqp_tests(devices, apps, device_keys, app_keys, dev_admin_id, dev_admin_key, app_admin_id, app_admin_key)
